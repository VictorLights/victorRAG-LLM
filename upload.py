    ...
    if file_path:
        with open(file_path, 'rb') as pdf_file:
            ...
            if current_chunk:  # Don't forget the last chunk!
                chunks.append(current_chunk)
            with open("vault.txt", "a", encoding="utf-8", errors='replace') as vault_file:
                for chunk in chunks:
...
root = tk.Tk()
root.title("LLama 3 Program Upload Interface")

# Set the window size
root.geometry("350x350")  # Width x Height

# Create a button to open the file dialog for PDF
pdf_button = tk.Button(root, text="Upload PDF", command=convert_pdf_to_text, font=('Helvetica', 14), height=2, width=20, fg='black')
pdf_button.pack(pady=20)

# Create a button to open the file dialog for text file
txt_button = tk.Button(root, text="Upload Text File", command=upload_txtfile, font=('Helvetica', 14), height=2, width=20, fg='black')
txt_button.pack(pady=20)

# Create a button to open the file dialog for JSON file
json_button = tk.Button(root, text="Upload JSON File", command=upload_jsonfile, font=('Helvetica', 14), height=2, width=20, fg='black')
